...
プレゼンテーション層でのログ仕込みと、インフラストラクチャ層（リポジトリ、サービス）へのログ仕込みを行う。[\[http://localhost:500 ...\]](http://localhost/graphql)

#### プレゼンテーション層

- GraphQL クエリ/ミューテーションの実行開始・終了
- エラーフィルターでの例外キャッチ
- 認証・認可の成功/失敗

#### インフラストラクチャ層

- リポジトリでのデータベースアクセス（クエリ実行時間含む）
- 外部サービス（JWT、パスワードハッシュ）の呼び出し
